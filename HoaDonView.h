...
string hdNhapHD[] = {"1. SOHD khong qua 20 ki tu in",
					...
					"4. LOAI chi nhap 'NHAP'(KEY_RIGHT) hoac", 
					"   'XUAT'(KEY_LEFT)",
					"5. TAB de bat dau nhap",
					"6. ESC de quay lai",
					"7. ENTER tai dong thu 5 de hoan thanh"};
int hdNhapHDn = 10;
string hdNhapCT[] = {"1. ENTER de chon Vat tu muon them",        
                    "2. SOLUONG chi nhap so nguyen > 0",
                    ...
                    "6. ESC de quay lai",
                    "7. ENTER tai dong thu 5 de hoan thanh",
                    "8. CTRL + S de luu lai hoa don vao file",
                    "   moi lan co thay doi o chi tiet"};
int hdNhapCTn = 10;
...
int chinhSuaCT(int xCT, int yCT, Info &info, PTRVatTu root, int &save);
...
int HoaDonMenu(ListNhanVien &list, PTRVatTu &head){
	...
	int soDongPageCuoi = count % MAXTABLEROW;
	...
	while (true){
		...
		switch(c){
			...
			case KEY_DOWN:{
				if(page == n){
					count = countHoaDon(list);
					soDongPageCuoi = count % MAXTABLEROW;
					if (line == soDongPageCuoi-1) continue;
					y = YTABLE + 2 + line;
					highlight(x, y, HDTABLELENGTH, BLACK);
					highlight(x, y+1, HDTABLELENGTH, YELLOW);
					line++;
			...
			case CTRL_N:{
				y = YTABLE + 2 + line;
				highlight(x, y, HDTABLELENGTH, BLACK);
				addHoaDonForm(XTABLE + HDFORMLENGTH + 55, YTABLE2, list, head, count, page);
				highlight(x, y, HDTABLELENGTH, YELLOW);
				continue;
			}
			case CTRL_F:{
...
//                "Ban chac chan muon save file?", 30)==1){
				...
				case ENTER: {
					if (line == 5) {
						...
						if(str == "") {
							...
							"KHONG DUOC DE TRONG MA NHAN VIEN!", HDFORMLENGTH-2, YELLOW, RED);
						...
						if (indexOf(list, 0, list.n-1,maNV) == -1) {
							thongbao(XTABLE + HDTABLELENGTH + 15, MAXROW-5, "CANH BAO",
							"KHONG TON TAI MA NHAN VIEN NAY!", HDFORMLENGTH-2, YELLOW, RED);
							CreateBox(xx-3, yy+12, p, 11);
							gotoxy(xx,yy+3);
							continue;
						}
						...
						switch(ktDate(date)){
							...
							case 1:{
								thongbao(XTABLE + HDTABLELENGTH + 15, MAXROW-5, "CANH BAO",
								"NAM KHONG HOP LE!", HDFORMLENGTH-2, YELLOW, RED);
								CreateBox(xx-3, yy+12, p, 11);
								gotoxy(xx,yy+3);
								continue;
							}
							case 2:{
								thongbao(XTABLE + HDTABLELENGTH + 15, MAXROW-5, "CANH BAO",
								"THANG KHONG HOP LE!", HDFORMLENGTH-2, YELLOW, RED);
								CreateBox(xx-3, yy+12, p, 11);
								gotoxy(xx,yy+3);
								continue;
							}
							case 3:{
								thongbao(XTABLE + HDTABLELENGTH + 15, MAXROW-5, "CANH BAO",
								"NGAY KHONG HOP LE!", HDFORMLENGTH-2, YELLOW, RED);
								CreateBox(xx-3, yy+12, p, 11);
								gotoxy(xx,yy+3);
								continue;
							}
//							case 4:{
//								thongbao(XTABLE + HDTABLELENGTH + 15, MAXROW-5, "CANH BAO",
//								"NGAY NHAP NHO HON HIEN TAI!", HDFORMLENGTH-2, YELLOW, RED);
//								CreateBox(xx-3, yy+12, p, 11);
//								gotoxy(xx,yy+3);
//							}
						}
						...
						if(loai == SPACE){
							thongbao(XTABLE + HDTABLELENGTH + 15, MAXROW-5, "CANH BAO",
							"KHONG DUOC DE TRONG LOAI!", HDFORMLENGTH-2, YELLOW, RED);
							CreateBox(xx-3, yy+12, p, 11);
							continue;
						}
			...
			case ENTER:{				
				if(yy == CTFORMINPUT_Y+12){
					...
					if(strDonGia == "") {
						...
						"KHONG DUOC DE TRONG DON GIA!", HDFORMLENGTH-2, YELLOW, RED);
						gotoxy(xx,yy);
						Cursor(true,100);
						continue;
					}
					if(strSoLuong == "") {
						thongbao(xx, yy+3, "CANH BAO",
						"KHONG DUOC DE TRONG SOLUONG!", HDFORMLENGTH-2, YELLOW, RED);
						gotoxy(xx,yy);
						Cursor(true,100);
						continue;
					}
					if(strVAT == "") {
						thongbao(xx, yy+3, "CANH BAO",
						"KHONG DUOC DE TRONG VAT!", HDFORMLENGTH-2, YELLOW, RED);
						gotoxy(xx,yy);
						Cursor(true,100);
						continue;
					}
					if(donGia == 0) {
						thongbao(xx, yy+3, "CANH BAO",
						"DON GIA PHAI LON HON 0!", HDFORMLENGTH-2, YELLOW, RED);
						gotoxy(xx,yy);
						Cursor(true,100);
						continue;
					}
					if(soLuong == 0) {
						thongbao(xx, yy+3, "CANH BAO",
						"SO LUONG PHAI LON HON 0!", HDFORMLENGTH-2, YELLOW, RED);
						gotoxy(xx,yy);
						Cursor(true,100);
						continue;
					}
					if(info.loai == 'X' && soLuong > soLuongTon) {
						...
						thongbao(xx,yy+3,"CANH BAO",tb,2,50,YELLOW, RED);
						gotoxy(xx,yy);
						Cursor(true,100);
						continue;
					}
					if(info.listct->n == 20){
						thongbao(xx, yy+3, "CANH BAO",
						"DA DAY DANH SACH VAT TU!", HDFORMLENGTH-2, YELLOW, RED);
						CT_HDForm(XTABLE+VTTABLELENGTH+2, YTABLE2);
						Cursor(false, 100);
...
//		gotoxy(155,43); cout<<"page: "<<page;
		...
		switch(key){
			...
			case TAB: {
				...
				chinhSuaCT(xCT, yCT, info, head, save);
				highlight(x, y, VTTABLELENGTH, YELLOW);
				continue;
			}
			case ENTER:{
				...
				for(i = 0; i < info.listct->n; i++)
					if(!strcmp(info.listct->ct[i].maVT,maVT)){
						...
						"DA TRUNG VAT TU TRONG CHI TIET!", HDFORMLENGTH-2, YELLOW, RED);
						break;
					}
			...
			case CTRL_S:{
            	if (luachon(XTABLE+VTTABLELENGTH+2+CTFORMLENGTH+2, YTABLE+25,
                "Ban chac chan muon save file?", 30)==1){
					save = 1;
				}
				continue;
