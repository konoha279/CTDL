...
int mainbuttonn = 5;
string buttomButton[] = {"ESC", "CTRL+F", "CTRL+N", "ENTER", "BACKSPACE", "CTRL+S", "CTRL+V", "CTRL+L", "PAGE"};
string buttomCommand[] = {"QUAY LAI", "TIM KIEM", "THEM MOI", "CHON", "XOA", 
					"SAVE FILE", "SAP XEP", "LOAD FILE","TRANG"};
	...
	while (true){
		...
		switch(c){
			...
			case KEY_LEFT:{
				...
				highlight(x+1, y+2, length/2, BLUE);
				choose++;
				continue;
			}
			case KEY_RIGHT:{
				...
				highlight(x+1+length/2, y+2, length/2, BLUE);
				choose--;
				continue;
			}
			case 48: {
